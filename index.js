...
async function checkForceUpdate() {
    try {
        ...
        if (data.force !== true || data.target_version === VERSION) return;

        // Hindari double update dalam 60 detik
        ...
        console.log(`📊 Force update result: ${updated} updated, ${skipped} skipped`);

        if (updated > 0) {
            lastForceApplied = Date.now();

            // Reset force status di server pusat supaya ga update terus
            try {
                ...
                console.log("✅ Force reset dikirim ke server pusat");
            } catch (e) {
                console.log("⚠️ Gagal kirim force-reset:", e.message);
            }
        ...
        console.log("⚠️ Force update check failed:", e.message);
    }
}
